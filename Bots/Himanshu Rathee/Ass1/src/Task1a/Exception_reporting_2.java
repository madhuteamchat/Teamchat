...
public class Exception_reporting_2 {
	...
	public void onCreateKeyword(TeamchatAPI api) {
		Form f = api.objects().form();
			...
			f.addField(api.objects().input().name("productcode").label("Product Code"));
		api.perform(
	...
	public void ondisplay(TeamchatAPI api){
		...
		Room x = api.context().create().setName("Data Entry").add(Admin).add(sender);
		api.perform(x.post(new PrimaryChatlet().setQuestion("Below threshold sales for " +productcode+" of " +sales)));
	}
	...
	public static void main(String[] args) {
		TeamchatAPI api = TeamchatAPI.fromFile("teamchat.data")
				...
				.setPassword("p@$$word4") 
				.startReceivingEvents(new Exception_reporting_2());
			}
	}
