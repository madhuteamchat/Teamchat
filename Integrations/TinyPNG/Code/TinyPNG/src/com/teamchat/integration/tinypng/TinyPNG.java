...
import com.teamchat.integration.tinypng.utils.Utility;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (connection.getResponseCode() == 201) {
				// Compression was successful, retrieve output from location
				// header.
			...
			api.perform(api
					...
					.post(new PrimaryChatlet()
							.setQuestionHtml(Utility.file.replace("__filePath", configProps.getProperty("filepath"))
										.replace("__nFileName", nFileName))));
		} catch (Exception ex) {
