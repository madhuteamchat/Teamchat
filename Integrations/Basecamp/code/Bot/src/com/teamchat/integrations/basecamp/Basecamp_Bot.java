			...
			api.perform(api
					...
					.post(new PrimaryChatlet()
							.setQuestionHtml("<h4 style=\"display:block;margin:0 auto;\">Login with Basecamp</h4>"
									...
									+ "https://launchpad.37signals.com/authorization/new?client_id=d48fa4605608e6bc3405232a71051aeb171eda35&redirect_uri=http%3A%2F%2Flocalhost%3A8080%2FBasecamp_servlet%2FRedirect_url&type=web_server\""
	...
	public Boolean isBasecampPresent(TeamchatAPI api) {
		...
		if (!api_init) {
			api.perform(api
					...
					.currentRoom()
					.post(new PrimaryChatlet()
							.setQuestion("Please type 'basecamp' to start using me !")));
			return false;
		}
		return false;
	}
