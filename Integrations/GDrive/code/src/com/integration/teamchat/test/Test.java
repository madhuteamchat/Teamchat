...
public class Test {
	...
	public void helpMethod(TeamchatAPI api)
	{
		...
		api.perform(api.context().currentRoom().post( 
				new PrimaryChatlet().setQuestionHtml(html)));		
	}
	...
	public void connectGDoc(TeamchatAPI api) {
		...
		sname=sname.replace('.', '_');
		String html="<a href='http://interns.teamchat.com:8084/GDrive/GDocBotServlet?client_id="
			     +client_id+"&client_secret="+client_secret+"&name="+sname+"&email="+email+"' target=\'_blank\'> Login </a>";
...
//Creating a chatlet to get/download the files present in the drive 	
	...
	try {
	    ...
	    for (File file : results) {
	         ...
	         html += "<a href='"+res+"' target=blank>"+file.getTitle()+"</a><br>";
	...
	public  List<com.google.api.services.drive.model.File> getFiles(Drive service) throws IOException {
	...
	// Function to get the url of files present in the drive so that we can download the file present in the file list
