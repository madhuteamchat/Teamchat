...
public class func {
	...
	public void ConnectToSlack(TeamchatAPI api) throws IllegalStateException,
		...
		api.perform(api
				...
				.post(new PrimaryChatlet()
						.setQuestionHtml("<a href='https://slack.com/oauth/authorize?client_id=5090557084.5108581326&redirect_uri=http://localhost:8080/slack_auth/slack_auth&scope=identify,read,post,client,admin' target='_blank'>Click Here to authorize</a>")));
		System.out.println("Haha, wait till I find your authorization info.");
		
		Thread.sleep(30000); //You can find a better way, Amey
		start(api);
		// To open new pop-up:
		// onClick="MyWindow=window.open('https://slack.com/oauth/authorize?client_id=5090557084.5108581326&redirect_uri=http://localhost:8080/slack_auth/slack_auth&scope=identify,read,post,client,admin','MyWindow',width=600,height=300);
		// return false;
	...
	public void start(TeamchatAPI api) throws IllegalStateException,
		...
		try (FileReader reader = new FileReader("/home/gupshup/code.properties")) {
		...
		if (!value) {
			...
			token = acc.get("access_token").toString(); //This token is unique to the user
	...
	public void sendMessage(TeamchatAPI api) throws IllegalStateException,
		...
		String actualmessage = msgToSend.replace(" ", "%20").replace("?", "%3F"); //For get request to url
	...
	public void userlist(TeamchatAPI api) throws ClientProtocolException,
		...
		if (ok) {
			...
			api.perform(api
					.context()
					.currentRoom()
					.post(new PrimaryChatlet().setQuestion("These are the users in your Slack team:" + listallnames)));
	...
	public void Query(TeamchatAPI api) throws ClientProtocolException,
		...
		HttpClient client = new DefaultHttpClient();
		HttpGet request = new HttpGet(
				"https://slack.com/api/search.messages?token="+token +"&query="
						+ query + "&pretty=1");
		...
		if (matches.length() == 0) {
			...
			for (int i = 0; i < matches.length(); i++) {
				...
				String poster = (String) name2.get("username"); //Who posted message

				JSONObject channel = name2.getJSONObject("channel"); //In which channel it was posted
				String channelname = (String) channel.get("name");

				String text = (String) name.get("text"); //What was posted
			
					strtodisp.append("User ").append(poster).append(" had posted ").append(text).append(" in channel ").append(channelname).append("<br>");
				}
