...
System.err.println(rmid);
	...
	public void setdata(TeamchatAPI api) throws SQLException {
		...
		String App_key = api.context().currentReply().getField("App_key");
			...
			{
				DBHandler datacng = new DBHandler();
				datacng.stmt.executeUpdate("UPDATE Bot.pingdom_auth"+" SET username ='"+ Username +"'"+", pass ='"+ Password +"'"+", appkey ='"+ App_key +"'"+" where roomId='" + roomId + "'");
		...
		else
		{
			...
			else
			{
				...
				for (Check check : checks.getChecks()) {
					...
					fo.addField(api.objects().select().name("Checks"+ String.valueOf(count))
							.label("Select check "+String.valueOf(count)+":").addOption(check.getHostname()).addOption("none"));
				...
				api.perform(api
						...
						.post(new PrimaryChatlet()
								.setQuestionHtml("<h5><b>You are logged in successfully!!</b></h5>"
										+ "<br /><b>Select Checks from below :</b>")
	...
	public void addcheck(TeamchatAPI api) throws IOException {
		...
		else
		{
            ...
            f.addField(api.objects().select().name("type")
					.label("Select any type :").addOption("http").addOption("https"));
			
					prime.setQuestionHtml("<h5><b>Enter the Check details below:</b></h5>")
							.setReplyScreen(f).setReplyLabel("Enter").alias("sendreqadd");
					api.perform(api.context().currentRoom().post(prime));
...
String del[] = new String[count];

		for (Check check : checks.getChecks()) {
			if(check.getHostname().equals(chks[k]))
			{
				del[k]=check.getId().toString();
			}
			else
			{
	...
	public void sendreqadd(TeamchatAPI api) throws IOException{
		...
		String type = api.context().currentReply().getField("type");
		
		pingdomintegrator ob1 = new pingdomintegrator();
		String resp = ob1.addChecks(Username, Password, App_key, host, name , type);
	if (resp.equals("Error")) {
		PrimaryChatlet prime = new PrimaryChatlet();
		api.perform(api
				.context()
...
//color="#24970D";---green
		for (Check check : checks.getChecks()) {
			...
			prime1.setQuestionHtml("<h4><b>Host Details "+String.valueOf(s)+":</b></h4>"+"<ul type=\"square\"; style=\"color:#359FD8\";><li><a1 style=\"color:black\";><b>Name- </b></a1><a2 style=\"color:#359FD8\";>" + check.getName() + "</a2></li><li><a3 style=\"color:black\";><b>Hostname- </b></a3><a4 style=\"color:#359FD8\";>" + check.getHostname() + "</a4></li><li><a5 style=\"color:black\";><b>Status - </b></a5><a6 style=\""+ color +"\";>" + check.getStatus() + "</a6></li><li><a7 style=\"color:black\";><b>Last Response Time - </b></a7><a8 style=\"color:#359FD8\";>" + check.getLastresponsetime() + "</a8></li><li><a9 style=\"color:black\";><b>Type - </b></a9><a10 style=\"color:#359FD8\";>" + check.getType() + "</a10></li></ul>");
