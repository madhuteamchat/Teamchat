...
public class ChooserResponse extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ChooserResponse() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i=0;i<data.length();i++) {
			...
			if (temp > 1024)
				size = new DecimalFormat("#.#").format((((double)(data.getJSONObject(i).getLong("bytes"))/1024/1024))) + "MB";
			else
				size = new DecimalFormat("#.##").format((((double)(data.getJSONObject(i).getLong("bytes"))/1024))) + "KB";
			
			String icon = null;
			String thumbnail = null;
			if (type.equals("png") || type.equals("jpg") || type.equals("jpeg") || type.equals("mp4")) {
				thumbnail = data.getJSONObject(i).getString("thumbnailLink");
				link = createEmbeddedLink(link, "File", "http");
				api.perform(r.post(new PrimaryChatlet()
						.setQuestionHtml("<div align=\"center\"><a href=\'" + link + "\' target=_blank><img src=\'" + thumbnail + "\'><br/>Name: " + name + "<br/>Size: " + size + "</a></div>")));
			} else {
				icon = data.getJSONObject(i).getString("icon");
				link = createEmbeddedLink(link, "File", "http");
				api.perform(r.post(new PrimaryChatlet()
						.setQuestionHtml("<div align=\"center\"><a href=\'" + link + "\' target=_blank><img src=\'" + icon + "\'><br/>Name:" + name + "<br/>Size: " + size + "</a></div>")));
			}
		}
	}
