...
public class Mainbot
{
		...
		} else
		{
			...
			api.perform(api
					...
					.post(new PrimaryChatlet().setQuestionHtml("Follow the instructions given below.We need you to get your access token one time.").setQuestionHtml(
							...
							.setQuestionHtml("You need to get your notification token as well to enable notifications for room.<br/>INSTRUCTION-Go To Edit Profile.Click on API access.Enter your password.Select all scopes and create token.Copy it and paste here.")));
	...
	public void onstop(TeamchatAPI api) throws IOException{
		...
		// Deletes webhook
				 ...
				 .url("http://api.hipchat.com/v2/room/" + id + "/webhook/" + webid + "")
				 .delete(null)
