...
public class EverNote {
	...
	String token;
	String nbGuidArray[] = null, nbArray[] = null, nArray[] = null,
	...
	public void help(TeamchatAPI api) {
		...
		print = print
				+ "<b>myevernote</b>:To perform your desired task from the dropdown<br>";
	...
	public NoteStoreClient create(TeamchatAPI api, String mail,
		...
		if (access_token != null) {
			...
			api.perform(api.context().currentRoom()
					.post(new TextChatlet("Not connected. Try again.")));
			return null;
		}
	}
		...
		try {
			if (task.equals("List NoteBooks")) {
				...
				if (notebooks.size() > 0) {
					...
					nb = nb + "</ol>";
					api.perform(api.context().currentRoom()
							.post(new PrimaryChatlet().setQuestionHtml(nb)));
				} else {
					api.perform(api.context().currentRoom()
							.post(new TextChatlet("No Notebooks to show.")));
				}
				...
				if (notebooks.size() < 1) {
					api.perform(api.context().currentRoom()
							.post(new TextChatlet("No Notebooks to edit")));
				} else {
					for (Notebook notebook : notebooks) {
				...
				if (notebooks.size() > 0) {
					...
					api.perform(api
							...
							.post(new PrimaryChatlet()
									.setQuestionHtml(
											"Fill the details to create note")
									.setReplyScreen(
											api.objects()
													.form()
													.addField(
					...
					api.perform(api
							...
							.post(new TextChatlet(
									"No notebooks. First create a notebook then you can create note.")));
				}
				...
				api.perform(api.context().currentRoom()
						.post(new TextChatlet("Please select a valid option.")));
			}
		} catch (Exception e) {
	...
	@OnAlias("edit")
	public void selectNoteToEdit(TeamchatAPI api) throws EDAMUserException,
		...
		if (nBook.length() > 0) {
			...
			for (Notebook notebook : notebooks) {
				if (notebook.getName().equals(nBook)) {
					...
					if (notes.size() > 0) {
						for (Note note : notes) {
							Note fullNote = noteStore.getNote(note.getGuid(),
						...
						api.perform(api
								...
								.post(new PrimaryChatlet()
										.setQuestionHtml(
												"Select the note to be modified")
										.setReplyScreen(
												api.objects()
														.form()
														.addField(
						...
						api.perform(api
								...
								.currentRoom()
								.post(new TextChatlet(
										"There are no Notes to edit in this notebook")));
					}
				}
			}
		} else {
			api.perform(api.context().currentRoom()
					.post(new TextChatlet("No Notebook is selected")));
		}
	}

	@OnAlias("edit1")
	public void showContentToEdit(TeamchatAPI api) throws EDAMUserException,
		...
		if (noteName.length() > 0) {
			...
			for (Notebook notebook : notebooks) {
				if (notebook.getName().equals(nBook)) {
					...
					List<Note> notes = noteList.getNotes();
					for (Note note : notes) {
						Note fullNote = noteStore.getNote(note.getGuid(), true,
								true, false, false);
					...
					String print = "Your Current content is : <br>" + data
							+ "<br><br>Enter your modified conent";
					api.perform(api
							.context()
							.currentRoom()
							.post(new PrimaryChatlet()
			...
			api.perform(api.context().currentRoom()
					.post(new TextChatlet("Please select a note to edit.")));
		}
	}

	@OnAlias("edit2")
	public void editNote(TeamchatAPI api) throws EDAMUserException,
			EDAMSystemException, EDAMNotFoundException, TException {
	...
	private void listReminders(TeamchatAPI api) throws EDAMUserException,
			EDAMSystemException, TException, EDAMNotFoundException {
		...
		if (print.length() > 0) {
			...
			api.perform(api.context().currentRoom()
					.post(new TextChatlet("No reminders to display.")));
		}
	}
	...
	@OnAlias("notebook")
	public void createNoteBook(TeamchatAPI api) throws EDAMUserException,
		...
		if (nbName.length() > 0) {
			...
			api.perform(api.context().currentRoom()
					.post(new TextChatlet("No name has been entered.")));
		}
	}

	@OnAlias("create")
	public void createNote(TeamchatAPI api) throws EDAMSystemException,
			EDAMNotFoundException, TException, EDAMUserException {
