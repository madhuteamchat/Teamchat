	...
	public void help(TeamchatAPI api) {
		api.perform(api
				...
				.post(new PrimaryChatlet()
						.setQuestionHtml("<div align='center'><img width='90px' height='90px' src='https://yt3.ggpht.com/-CepHHHB3l1Y/AAAAAAAAAAI/AAAAAAAAAAA/Z8MftqWbEqA/s900-c-k-no/photo.jpg'/></div><br/>"
	...
	public void link(TeamchatAPI api) {
		...
		if (acc == 0) {
			...
			api.perform(api
					.context()
					.currentRoom()
					.post(new TextChatlet("already connected")));
			/*
	...
	public void profile(TeamchatAPI api) {
		...
		if (acc == 0) {
			api.perform(api
					.context()
					.currentRoom()
					.post(new TextChatlet("please authenticate first!")));
			...
			if (response.getCode() == 200) {
				...
				html = html.replace("__link",
						j1.getJSONObject("siteStandardProfileRequest")
								.getString("url"));
				html = html.replace("__name", j1.getString("firstName") + " "
				...
				api.perform(api
						.context()
						.currentRoom()
						.post(new TextChatlet("you are not authorized, please authorize first!")));
			} else {
				api.perform(api
						.context()
						.currentRoom()
	...
	public void post(TeamchatAPI api) {
		...
		if (acc == 0) {
			...
			f.addField(api.objects().input().label("URL")
					.addRegexValidation(".+", "please enter a value")
					.name("url"));
			f.addField(api.objects().input().label("Image URL (Max. file size supported: 4.5mb)")
					.addRegexValidation(".+", "please enter a value")
					.name("imageUrl"));
	...
	public void details(TeamchatAPI api) throws IOException {
		...
		if (response.getCode() == 201) {
			api.perform(api
					...
					.currentRoom()
					.post(new PrimaryChatlet()
							.setQuestionHtml("successfully posted on linkedin, check: <a href='"
									+ update
									+ "' target='_blank'>'"
									+ title
									+ "'</a>")));
			...
			api.perform(api
					.context()
					.currentRoom()
					.post(new TextChatlet("something went wrong, try again!")));
		}
	}
		...
		if (acc == 0) {
			...
			if (success == 1) {
				api.perform(api
						.context()
						.currentRoom()
						.post(new TextChatlet("linkedin access has been removed, to allow send 'help' keyword again")));
			} else {
				api.perform(api
						.context()
						.currentRoom()
