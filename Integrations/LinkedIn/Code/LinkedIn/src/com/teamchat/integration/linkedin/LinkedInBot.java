...
public class LinkedInBot {
	...
	public void link(TeamchatAPI api) {
		...
		if (acc == 0) {
			...
			api.perform(api
					...
					.currentRoom()
					.post(new PrimaryChatlet()
							.setQuestionHtml(Data.auth.replace("__auth", authUrl))));
		} else {
	...
	public void profile(TeamchatAPI api) {
		...
		if (acc == 0) {
			...
			request = new OAuthRequest(Verb.GET, Data.people1);
			...
			if (response.getCode() == 200) {
				...
				request = new OAuthRequest(Verb.GET, Data.people2);
				service.signRequest(t, request); // the access token from step 4
				response = request.send();
	...
	public void details(TeamchatAPI api) throws IOException {
		...
		request = new OAuthRequest(Verb.POST, Data.share);
		...
		if (response.getCode() == 201) {
			...
			api.perform(api
					...
					.currentRoom()
					.post(new PrimaryChatlet()
							.setQuestionHtml(Data.update.replace("__update", update).replace("__title", title))));
		} else {
			api.perform(api.context().currentRoom()
					.post(new TextChatlet("oops! something went wrong, linkedin is not able to process your request<br/>please try again,"
