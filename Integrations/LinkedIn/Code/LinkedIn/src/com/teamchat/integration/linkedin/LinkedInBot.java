	...
	public void link(TeamchatAPI api) {
		...
		if (acc == 0) {
			...
			api.perform(api
					...
					.currentRoom()
					.post(new PrimaryChatlet()
							.setQuestionHtml(Utility.auth.replace("__auth", authUrl))));
		} else {
	...
	public void details(TeamchatAPI api) throws IOException {
		...
		if (response.getCode() == 201) {
			...
			api.perform(api
					...
					.currentRoom()
					.post(new PrimaryChatlet()
							.setQuestionHtml(Utility.update.replace("__update", update).replace("__title", title))));
		} else {
			api.perform(api.context().currentRoom()
