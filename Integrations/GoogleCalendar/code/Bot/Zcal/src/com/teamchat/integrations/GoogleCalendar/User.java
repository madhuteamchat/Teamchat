...
public class User {
		...
		{
		String query = "Select * from Bot.CalendarTable where email='"+email+"'" ;
		...
		{ 
			String query="Update Bot.CalendarTable SET access_token=? where email=?";
	...
	{
		String query="Insert into Bot.CalendarTable"+"(email,access_token,expires_in) values"+"(?,?,?)";
		System.out.println(query);
		try{
			Class.forName(DRIVER1);
			con = (Connection) DriverManager.getConnection(URL, USER, PASSWORD);
	...
	{
		String query="Delete from Bot.CalendarTable where email=?";
		System.out.println(query);
		try{
			Class.forName(DRIVER1);
			con = (Connection) DriverManager.getConnection(URL, USER, PASSWORD);
