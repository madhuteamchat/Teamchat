...
public class Test {
	/*public static String bot = "bhushanbot@gmail.com";
	public static String pwd = "qwerty qwerty";
   ...
   public static TeamchatAPI api1;
	...
	/*@OnKeyword("calendar")
	public void calendar(TeamchatAPI api) {
	     api1 = api;
		api.perform(api
				.context()
				.currentRoom()
				.post(new PrimaryChatlet()
						.setQuestion("Select Your Option:")
						.setReplyScreen(
								api.objects()
										.form()
										.addField(
	...
	public void opt(TeamchatAPI api) throws IOException {
		...
		Calendar calendar = getCalendar(api.context().currentSender().getEmail());
	...
	private void onListReceived(TeamchatAPI api, List<Event> items)  {
		...
		System.out.println(Arrays.toString(timed));  	//TIME of corressponding event

		for(int j=0;j<s;j++)
		{
			hr=Integer.parseInt(timed[j].substring(0, 2));
			min=Integer.parseInt(timed[j].substring(3, 5));
			mydate=(dated[j].substring(0, 10));				// mydate is in yyyy-mm-dd format
			//evename=summ[j];
			Calendar1 a = new Calendar1();
			System.out.println("Time:" + hr + ": "+ min+"Date:" +" "+mydate);
	...
	//		a.checkdate(mydate,hr,min);
		...
		for (Event event : items) {
			...
			html +="<li><h3><b><font color='#159CEB'>"+ event.getSummary()+ "</font></b></h3>"  + "<h4><font color='black'>"+start.toString().replace("T","          ").substring(0, 25)+"</font></h4>" + "</li>";
