	...
	public void getdata(TeamchatAPI api) throws IOException {
		...
		if (resp.equals("Error")) {
			...
			for (Datum datas : data.getData()) {
				...
				prime.setQuestionHtml("<h6><b>Image "+(k)+":</b></h6>" + "<img src=\"" + or.getUrl()
						+ "\" height=\"160\" width=\"280\">").setReplyScreen(fo).setReplyLabel("TAG")
						.alias("gettag");
				api.perform(api.context().currentRoom().post(prime));
				//urlpge[k] = or.getUrl();
...
//		System.err.println(chtid);
		...
		{   DBHandler ob5 = new DBHandler();
			if(ob5.dchk(emailid,tag)==true)
			{
				...
				datacng.stmt.executeUpdate("UPDATE Bot.giphy_auth"
						+ " SET url ='"	+ urlpge[i] + "'" + " where emailid='" + emailid + "' and tag='" + tag +"'");
				datacng.conn.close();
		...
		else
		{
			...
			fo.addField(api.objects().select()
					...
					.addOption(tags[0]).addOption(tags[1]).addOption(tags[2]).addOption(tags[3]).addOption(tags[4]));
		
			PrimaryChatlet prime = new PrimaryChatlet();
			prime.setQuestionHtml("<h4><b>Sorry, Your storage capacity is full!!!</b></h4>"
					+ "<br /><b>Selct a tag from the drop-down list in the reply to make some free space.").setReplyScreen(fo).setReplyLabel("Reply").alias("getfulltag");
			api.perform(api.context().currentRoom().post(prime));
		}
	}
		...
		{ DBHandler ob5 = new DBHandler();
			if(ob5.dchk(emailid,this.tag)==true)
			{
				...
				ui.stmt.executeUpdate("UPDATE Bot.giphy_auth"
						+ " SET url ='"	+ urlpge[i] + "'" + " where emailid='" + this.emailid + "' and tag='" + this.tag +"'");
				ui.conn.close();
				PrimaryChatlet prime1 = new PrimaryChatlet();
				prime1.setQuestionHtml("<h4><b>TAG successfully replaced!!!</b></h4>");
				api.perform(api.context().currentRoom().post(prime1));
...
//					
			...
			else if ((ttl - (offset + limit)) < 5 && (ttl - (offset + limit)) != 0) {
